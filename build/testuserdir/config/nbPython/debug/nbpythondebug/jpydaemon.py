...
#      self.dbgTrc.write(message + '\n');
    ...
    def user_line(self, frame):
        ...
        xmlLine =  [ '<LINE',
                               'cmd="'+ builtins.str(self.cmd)+'"' ,
                               'fn="'+ _utils.removeForXml(fn)+'"' ,
                               'lineno="'+builtins.str(frame.f_lineno)+'"' ,
                               'name="' + _utils.removeForXml(name) + '"' ,
    ...
    def populate_exception( self , exc_stuff):
        ...
        if ( self.exceptionRaised == 0 ): # exception not yet processed
            ...
            value = exc_stuff[1]
            tb  = exc_stuff[2]
            excTrace = builtins.str( traceback.format_exception( extype , value , tb ) )
            ...
            if issubclass( extype , SystemExit):
                ...
                content = builtins.str(value)
                error = value[0]
                compd = value[1]
                content = 'SOURCE:SYNTAXERROR:"'+\
                       builtins.str(compd[0])+ '":('+\
                       builtins.str(compd[1])+','+\
                       builtins.str(compd[2])+\
                       ')'+':'+error
            elif issubclass(extype,NameError):
                content = 'SOURCE:NAMEERROR:'+builtins.str(value)
            elif issubclass(extype,ImportError):
                content = 'SOURCE::IMPORTERROR:'+builtins.str(value)
            else:
                content = builtins.str(value)
            ...
            self.lastLineException = ['<EXCEPTION',
                                    ...
                                    '"' ,
                                    '/>']
            self.send_client_exception( builtins.str(self.cmd) , _utils.removeForXml(content+excTrace) )
    ...
    def user_return(self, frame, retval):
        ...
        self.sendBack( [  '<RETURN',
                                  'cmd="'+builtins.str(self.cmd)+'"' ,
                                  'fn="'+ _utils.removeForXml(fn)+'"' ,
                                  'retval="'+_utils.removeForXml(builtins.str(retval))+'"' ,
                                  '/>'] )
...
class MainThread (threading.Thread) :
    ...
    def run ( self ) :
        ...
        exec(self._cmd, self._myglobals,self._mylocals)
...
class Suspended :
    ...
    def resume(self , curThread ):
        try :
            self._lock.acquire()
            if ( curThread in self._suspended ) :
    ...
    def listSuspended(self) :
        try :
            self._lock.acquire()
            returned = list(self._suspended.keys())
        finally :
            self._lock.release()
        return returned
    ...
    def isSuspended( self , curThread ):
        self._lock.acquire()
        suspended = curThread in self._suspended
        self._lock.release()
        return suspended
...
class JPyDbg(BdbClone) :
    ...
    def runIt(self, filename , verb ):
        ...
        locals_ = globals_
        statement = 'exec(compile(open("{}").read(), "{}", "exec"))'.format(filename, filename)
    ...
    def formatStackElement( self , element ):
        ...
        return _utils.removeForXml(fName + ' (' + builtins.str(line) + ') ')
    ...
    def dealWithVariables( self , frame , type , stackIndex  ):
        ...
        for mapElement in list(variables.items()):
            xmlVariables.append('<VARIABLE ')
            xmlVariables.append('name="'+ _utils.removeForXml(mapElement[0])+'" ')
            xmlVariables.append('content="'+ _utils.removeForXml(builtins.str(mapElement[1]))+'" ')
    ...
    def isComposite( self , value ):
        if isinstance(value , dict ) :
            ...
            return 0
        elif not ( isinstance(value , bytes ) or \
    ...
    def populateVariable( self , xmlVariables , name , value ) :
        ...
        xmlVariables.append('content="'+ _utils.removeForXml(builtins.str(value)) +'" ')
    ...
    def parseSubCommand( self , command , frame , lthread ):
        ...
        if ( verb.upper() == "CMD" ):
        ...
        elif ( verb.upper() == "READSRC" ):
            self.populateCommandToClient( command ,
                                        self.dealWithRead( verb , arg )
                                      )
            self.cmd = FREEZE

        elif ( verb.upper() == "NEXT" ):
            self.cmd = NEXT
            ...
            self.set_next(frame)
        elif ( verb.upper() == "STEP" ):
            self.cmd = STEP
            lthread.additionalInfo.cmd=STEP
            # self.set_step()
        elif ( verb.upper() == "STEPOUT" ):
        ...
        if self._connection.connect() :
            ...
            if debuggee != None:
                ...
                if debuggeeArgs != None:
                    welcome.append(debuggeeArgs.join())
